...
# The following code gives us the hours, minutes and seconds of given milliseconds numbers
...
def main_get():
        return render_template('index.html', developer_name = 'Veysel ORS', not_valid = False)

@app.route('/', methods=['POST'])
def main_post():
    alpha = request.form['number']
    if not alpha.isdecimal():
        return render_template('index.html', developer_name = 'Veysel ORS', not_valid = True)
    if not (0 < int(alpha)):
        return render_template('index.html', developer_name = 'Veysel ORS', not_valid = True)
    return render_template('result.html', developer_name = 'Veysel ORS', milliseconds = alpha, result = convert(int(alpha)) )
